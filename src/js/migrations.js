...
function applyIntegrationDefaults(data, defaults = {}) {
  data.integrations = data.integrations || defaults.integrations || {
    enabled: false,
    defaultProviders: {
  ...
  data.priceHistory = Array.isArray(data.priceHistory) ? data.priceHistory : [];
  return data;
}

function applyHotelSelectionDefaults(data, defaults = {}) {
  data.hotelOptions = Array.isArray(data.hotelOptions) ? data.hotelOptions : clone(defaults.hotelOptions || []);
  return data;
}
